...
import { CalendarPlus, Check, Edit2, Trash2, X } from "lucide-react";
...
const Leaves = () => {
  ...
  const filteredLeaves = leaves.filter((leave) => {
    ...
    if (filterName && isManager && leave.employeeId?.name) {
      if (
        !leave.employeeId.name.toLowerCase().includes(filterName.toLowerCase())
      )
        match = false;
    }
    if (filterMonth) {
      ...
      if (
        leaveYear.toString() !== fYear ||
        leaveMonth.toString() !== parseInt(fMonth).toString()
      ) {
        match = false;
      }
    }
